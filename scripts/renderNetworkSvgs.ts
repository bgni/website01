...
import { applyTieredLayout } from "./layouts/tiered.ts";
import { typeColor } from "./graphLogic.ts";
import {
  forceCenter,
  forceCollide,
  forceLink,
...
const usage = () => {
  console.log(
    `Usage: deno run --allow-read --allow-write scripts/renderNetworkSvgs.ts [options]
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" role="img" aria-label="${
  ...
  <desc>Generated by scripts/renderNetworkSvgs.ts.</desc>
