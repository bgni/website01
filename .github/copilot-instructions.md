...
- Prefer mechanical refactors with shims; avoid behavior changes bundled with moves.
- Keep boundaries explicit:
  - Bootstrap resolves DOM and injects elements (no hard-coded selectors in renderers).
  - Controller owns lifecycles (e.g., `ResizeObserver`) and injects IO deps.
- D3 is a browser global; use `getD3()` from `scripts/lib/d3.ts` (no implicit global `d3`).
